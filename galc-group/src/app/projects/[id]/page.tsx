  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <InfoItem icon={Building2} label="Project Type" value={project.type} />
              <InfoItem icon={MapPin} label="Location" value={project.location} />
              <InfoItem icon={User} label="Client" value={project.client} />
              <InfoItem icon={Calendar} label="Start Date" value={formatDate(project.startDate)} />
              <InfoItem
                icon={Clock}
                label="End Date"
                value={project.endDate ? formatDate(project.endDate) : "En curso"}
              />
              <InfoItem icon={Building2} label="Total Area" value={`${project.squareMeters.toLocaleString()} m²`} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Budget and Progress</CardTitle>
            <CardDescription>Financial status and project progress</CardDescription>
          </CardHeader>
